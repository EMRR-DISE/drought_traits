...
unique(total_size$organism_code) #33 unique organism codes with size trait data (23 with only worms, so added 10 more)
#check how many taxa are still missing

#combine USGS/WoRMS/Nemesis size trait data -----
#data organized differently in GitHub, re-read in for now.
nemesis_size <- read.csv("./benthic/data_output/traits/benthic_traits_nemesis_size&physiology.csv")
usgs_size <- read.csv("./benthic/data_output/traits/benthic_traits_usgs_size.csv")
worms_size <- read.csv("./benthic/data_output/traits/benthic_traits_worms_size.csv")

unique(c(worms_size$organism_code,usgs_size$organism_code,nemesis_size$organism_code))
#53 of 64 taxa are represented by these 3 database pulls.